...
- **像优化器提供提示或在必要时屏蔽**。在连接操作中，你可以在数据表列表中的某个表名后面利用FORCE INDEX、USE INDEX或IGNORE INDEX限定词告诉服务器你想使用哪个索引，还可以使用STRAIGHT_JOIN强制优化器按照特定的顺序使用数据表。在正常情况下，MySQL优化器会按照能最快检索出数据的顺序来执行，但在少数情况MySQL优化器的选择并不一定是最优的。如果遇到这种情况，可以使用STRAGHT_JOIN覆盖优化器决定，使用STRAIGHT_JOIN的连接操作类似于一个交叉查询，但它将迫使数据表按照它们在FROM子句中出现的顺序联结查询。在这种情况下你应该选取数据行最少的表放在第一个，这里的原则是，让限制性最强的表选取操作最先执行，候选数据行的个数缩减的越快、越早，查询的性能就越高。
STRAIGHT_JOIN可以在两个地方使用，其一是在SELECT后面，这将会对所有的联结操作起作用；其二是放在FROM字句中。下面两条语句是等效的：
```
SELECT STRAIGHT_JOIN ... FROM t1 INNER JOIN t2 INNER JOIN t3 ...;
SELECT ... FROM t1 STRAIGHT_JOIN t2 STRAIGHT_JOIN t3 ...;
```
应该在使用和未使用STRAIGHT_JOIN的情况下分别测试一下查询的执行情况，STRAIGHT_JOIN可能起不到更好的作用。（可以用EXPLAIN查看执行计划，看MySQL是如何处理每一条语句的）。
- **尽量使用数据类型相同列进行比较**。对带有索引的数据列进行比较，如果他们数据类型相同，查询性能会高一些，如果数据类型不同，性能会相对低一些。
- **带索引的列在比较表达式中单独出现**。如果在函数调用中使用一个数据列，或者将数据列作为算数表达式中一部分，MySQL将不会使用索引，因为它必须对每一行数据进行计算。虽然有时是无法避免的，但在有些情况下可以重写查询语句来避免这种情况。比如下面这种情况，从查询结果来看是一样的，但是从查询性能来看就大不一样。
```
WHERE mycol * 2 < 4;
WHERE mycol < 4 / 2;
```
对一个WHERE子句来说，MySQL必须检索数据列mycol的所有值，然后乘以2，再将所得的结果与4比较。在这种情况下不能使用索引，因为数据列中每个值都要被检索后并计算后与后面的值进行比较。对第二个子句来说，优化程序会将表达式4/2简化为2，然后对mycol列使用索引，就能很快找出<2的值。
- **不要在LIKE模式的开始位置使用通配符**。在LIKE子句中的开始位置使用通配符，索引将不会生效。列入下面这种情况，即使该列创建了索引，索引也不会被使用：
```
WHERE col_name LIKE '%string%'
```
- **利用优化器的长处**。MySQL支持联结查询和子查询，但子查询的支持是从MySQL4.1才开始的。因此，在许多场合，优化器对联结查询优化的效果要比子查询的要好一些。
- **试验各种查询的变化格式，而且要多次运行它们**。当试验是个查询的变化格式（例如子查询和联结查询）时，每一种方式多运行几次。假如你对一个查询的两种方式只运行一次，你会发现第二种查询方式要快一些，实际上只是第一次的查询结果保存在磁盘的告诉缓存内。还要注意，应该在系统相对稳定时查询，这样可以避免系统上其他活动对查询结果造成影响。
- **避免过多的使用MySQL自动类型转换**。MySQL能够实现自动类型转换，但是如果你能避免这些类型转换，性能也许会更好。例如，如果数据列mycol是整数型的，下面这两个查询结果是一样的：
```
SELECT * FROM mytbl WHERE num_col = 4;
SELECT * FROM mytbl WHERE num_col = '4';
```
但是第二条语句包含类型转换，这个转换操作对整数和字符串进行了重复转换来完成比较操作，因而在性能方面会有损失。比较严重的问题是如果数据列mycol有索引，类型转换将导致索引不会被使用。
